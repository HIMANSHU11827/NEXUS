#!/usr/bin/env python3
"""Runtime validation for NEXUS AI TUI v2.1."""
import json
import os
import sys
import threading
...
# This is an executable smoke harness, not a pytest test module.  Pytest
# discovers it because of its historical filename; skip it at collection time
# so importing the test suite never starts a server or calls ``sys.exit``.
if __name__ != "__main__":
    try:
        import pytest
    except ImportError as exc:  # pragma: no cover - only relevant outside dev/test installs
        raise RuntimeError("tui.test_runtime must be executed as a script") from exc
    pytest.skip("manual TUI runtime smoke harness", allow_module_level=True)
...
AUTH_TOKEN = os.environ.get("NEXUS_DASHBOARD_TOKEN", "").strip() or "runtime-local"
...
def api_post(path, data, method="POST"):
    try:
        ...
        req = urllib.request.Request(f"{API}{path}", data=body,
            headers={"Content-Type": "application/json", **AUTH_HEADERS}, method=method)
