...
from core.code_intelligence.knowledge_graph import CodebaseKnowledgeGraph, CodeGraph
...
class AgentContextGenerator:
    """Create AGENTS.md/CLAUDE.md from verified repo structure."""

    def __init__(self, root_dir: str) -> None:
        self.root = os.path.abspath(root_dir)
        self.graph = CodebaseKnowledgeGraph(self.root)

    def generate(self, target: str = "AGENTS.md", graph: CodeGraph | None = None) -> str:
    ...
    def _safe_target(self, target: str) -> str:
        name = os.path.basename(target or "AGENTS.md")
        if name not in DEFAULT_TARGETS:
            raise ValueError("target must be AGENTS.md or CLAUDE.md")
        return name
