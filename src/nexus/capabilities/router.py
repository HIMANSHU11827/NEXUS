...
class IntentRouter:
    ...
    SIGNALS: Dict[str, Dict[str, Any]] = {
        ...
        "hive": {
            "patterns": [
                r"\b(hive|delegate|parallel|hive worker|spawn)\b",
        ...
        "strategy": {
            "patterns": [
                r"\b(strategy|roadmap|prioriti[sz]e|market|compete|competitor|defeat|ceo|vision|direction)\b",
                r"\b(product|growth|positioning|milestone|north star|a to z|end to end)\b",
            ],
            "weight": 1.2,
            "tools": ["roadmap", "evidence_ledger", "unified_graph", "benchmark"],
        },
    }
    ...
    def classify(
        ...
        try:
            ...
            from nexus.common.nexus_path import _ROOT
        ...
        if scores:
            ...
            # Keep chat startup deterministic. Calling an LLM just to classify
            # an LLM request can make the UI look frozen when providers are slow.
            best_intent = "chat"
            confidence = 0.9
            needs_tools = False
    ...
    def _detect_complexity(self, text: str, scores: Dict[str, float]) -> str:
        ...
        complex_keywords = ["plan", "design", "architect", "complex", "hard", "difficult", "step-by-step", "refactor", "implement", "build a new"]
        simple_keywords = ["hello", "hi", "glob", "cat", "ls", "grep", "simple", "status", "check", "run"]

        if any(w in text_lower for w in complex_keywords) and not any(w in text_lower for w in simple_keywords):
            return "complex"
