...
def test_deepseek_native_tool_envelope_is_parser_compatible():
    text = DeepSeekProvider._tool_envelope([
        {"id": "call_1", "function": {
...
def test_deepseek_generate_forwards_tools_and_returns_native_call(monkeypatch):
...
def test_deepseek_stream_buffers_native_tool_call_fragments(monkeypatch):
    provider = DeepSeekProvider()
    provider.api_key = "live-test-key"
