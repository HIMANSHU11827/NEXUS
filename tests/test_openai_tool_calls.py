...
def test_openai_native_tool_envelope_is_parser_compatible():
    text = OpenAIProvider._tool_envelope([
...
def test_openai_generate_forwards_tools_and_returns_native_call(monkeypatch):
...
def test_openai_stream_buffers_native_tool_call_fragments(monkeypatch):
    provider = OpenAIProvider()
    provider.api_key = "live-test-key"
