...
from nexus_compat import import_yaml
...
class DiagnosticRunner:
    ...
    def run(
        ...
        if include_gui or include_dashboard:
            diagnostics.append(self._check_gui_build())
    ...
    def _check_gui_build(self) -> Diagnostic:
        start = time.time()
        gui = os.path.join(self.root, "gui")
        if not os.path.isdir(gui):
            return Diagnostic("gui", "gui_build", False, "gui directory missing", self._elapsed(start))
        try:
            proc = subprocess.run(
                "npm run build",
                cwd=gui,
            ...
            return Diagnostic("gui", "gui_build", proc.returncode == 0, output if proc.returncode else "", self._elapsed(start))
        except Exception as exc:
            return Diagnostic("gui", "gui_build", False, str(exc)[:1000], self._elapsed(start))
