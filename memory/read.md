...
- **`MemoryManager`** (`memory/__init__.py`) is the single entry point for all memory systems:
  ...
  - Session files live at `.nexus/logs/sessions/<session_id>.json`; a fresh process resumes the most recent session automatically.
- **Episodic memory**: scored from `<root>/.nexus/v5/replays.jsonl` with Smallville-style recency·α + relevance·β + importance·γ weights; failures get top relevance/importance.
...
## Continuity
- `ContinuitySnapshot` dataclass with `as_prompt()` — surfaces only unfinished work backed by **durable evidence** (run contexts, `.nexus/v5/checkpoints` with non-terminal phases, `.nexus/workspace/todo.md`, task queue) via `inspect_continuity()`.
