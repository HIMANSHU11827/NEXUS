...
class AdaptiveMemoryGraph:
    ...
    def semantic_traverse(self, start_node_id: str, depth: int = 2) -> List[MemoryNode]:
        """
        [MULTI_HOP_REASONING]: Follows edges in the memory graph to find related context.
        Allows the agent to link failures to project components and historical fixes.
        """
        ...
        while to_visit:
            curr_id, curr_depth = to_visit.pop(0)
            ...
            if node and node.active:
                ...
                for neighbor_id in node.links:
                    if neighbor_id not in visited:
                        to_visit.append((neighbor_id, curr_depth + 1))
    ...
    def link_nodes(self, node_id_a: str, node_id_b: str):
        ...
        if node_id_a in self.nodes and node_id_b in self.nodes:
            if node_id_b not in self.nodes[node_id_a].links:
                self.nodes[node_id_a].links.append(node_id_b)
            if node_id_a not in self.nodes[node_id_b].links:
                self.nodes[node_id_b].links.append(node_id_a)
            self._save()
