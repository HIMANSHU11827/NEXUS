...
# NEXUS LoRA Checkpoint — PHASE_1_RAW

Raw (unpruned) PEFT LoRA adapter checkpoint from Phase 1 of the NEXUS local-brain fine-tuning pipeline (see `scripts/nexus_trainer.py` and `evolution/local_trainer/`). Retained as a reference baseline before further phases and GGUF conversion.
...
model = PeftModel.from_pretrained(base, "models/checkpoints/PHASE_1_RAW")
