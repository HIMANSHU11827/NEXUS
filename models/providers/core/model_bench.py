...
def load_tasks() -> List[Dict[str, Any]]:
    """Load per-task keyword + benchmark-weight tables from configure."""
    path = Path(__file__).resolve().parent.parent / "configure" / "model_tasks.json"
    raw: Any = None
    try:
        if path.exists():
...
def _profile_penalty(provider: Any) -> float:
    ...
    """
    try:
        from models.providers.core import profiles as _profiles_module
