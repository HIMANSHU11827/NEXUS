...
await checkForUpdates(
  'Run `npm install nexus-omni@latest` to update.',
);
...
if (args.server) {
  ...
  await startStandaloneServer(args.serverPort);
} else {
  logger(`Starting NEXUS Browser Bridge (MCP) v${VERSION}`);
  const {server, clearcutLogger} = await createMcpServer(args, {
    logFile,
  });
  const transport = new StdioServerTransport();
  await server.connect(transport);
  logger('NEXUS Browser Bridge connected');
  logDisclaimers(args);
