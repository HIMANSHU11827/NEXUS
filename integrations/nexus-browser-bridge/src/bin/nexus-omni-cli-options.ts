...
export const cliOptions = {
  ...
  browserUrl: {
    type: 'string',
    description:
      'Connect to a running, debuggable Chrome instance (e.g. `http://127.0.0.1:9222`). For more details see: https://github.com/ChromeDevTools/nexus-omni#connecting-to-a-running-chrome-instance.',
    alias: 'u',
  ...
  userDataDir: {
    type: 'string',
    description:
      'Path to the user data directory for Chrome. Default is $HOME/.cache/nexus-omni/chrome-profile$CHANNEL_SUFFIX_IF_NON_STABLE',
  ...
  chromeArg: {
    type: 'array',
    describe:
      'Additional arguments for Chrome. Only applies when Chrome is launched by nexus-omni.',
  },
  ignoreDefaultChromeArg: {
    type: 'array',
    describe:
      'Explicitly disable default arguments for Chrome. Only applies when Chrome is launched by nexus-omni.',
  },
  categoryEmulation: {
    type: 'boolean',
    default: true,
  ...
  server: {
    type: 'boolean',
    description: 'Launch as a standalone HTTP server (no MCP protocol).',
    default: false,
  },
  serverPort: {
    type: 'number',
    description: 'Port for the standalone HTTP server.',
    default: 3000,
  },
...
export function parseArguments(version: string, argv = process.argv) {
  const yargsInstance = yargs(hideBin(argv))
    .scriptName('npx nexus-omni@latest')
    .options(cliOptions)
    .check(args => {
    ...
    .example([
      ...
      [
        ...
        'Do not send usage statistics https://github.com/ChromeDevTools/nexus-omni#usage-statistics.',
      ],
      [
