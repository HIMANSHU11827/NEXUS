...
import {commands} from './nexus-browser-cli-options.js';
import {cliOptions, parseArguments} from './nexus-omni-cli-options.js';

await checkForUpdates(
  'Run `npm install -g nexus-omni@latest` and `nexus-browser start` to update and restart the daemon.',
);
...
const y = yargs(hideBin(process.argv))
  .scriptName('nexus-browser')
  .showHelpOnFail(true)
  .usage('nexus-browser <command> [...args] --flags')
  .usage(
    `Run 'nexus-browser <command> --help' for help on the specific command.`,
  )
  .option('sessionId', {
    type: 'string',
...
y.command(
  'status',
  'Checks if nexus-omni is running',
  y => y,
  async argv => {
    if (isDaemonRunning(argv.sessionId)) {
      console.log('nexus-omni daemon is running.');
      ...
      console.log('nexus-omni daemon is not running.');
    }
    process.exit(0);
  },
);
