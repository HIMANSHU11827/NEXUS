...
class UnifiedNexusGraph:
    ...
    def build(self, event_limit: int = 1000, include_code: bool = True) -> UnifiedGraph:
        ...
        self._ingest_hive_logs(graph, edge_keys, event_limit=event_limit)
    ...
    def _ingest_hive_logs(self, graph: UnifiedGraph, edge_keys: Set[tuple[str, str, str]], event_limit: int) -> None:
        hive_dir = os.path.join(self.root, "logs", "hive")
        if not os.path.isdir(hive_dir):
            return
        for root, _, files in os.walk(hive_dir):
            for filename in sorted(files)[:200]:
                abs_path = os.path.join(root, filename)
                rel_path = os.path.relpath(abs_path, self.root).replace("\\", "/")
                log_id = f"hive_log:{self._digest(rel_path)}"
                self._add_node(
                    graph,
                    log_id,
                    "hive_log",
                ...
                self._add_edge(graph, edge_keys, "store:hive_logs", log_id, "contains")
                if filename.endswith(".jsonl"):
                    for index, event in enumerate(self._read_jsonl_tail(rel_path, max(0, min(event_limit, 100)))):
                        event_id = f"hive_event:{self._digest(rel_path)}:{index}"
                        self._add_node(graph, event_id, "hive_event", event.get("type", event.get("event", "hive_event")), "hive_logs", metadata=event)
                ...
                if filename == "hive_manifest.json":
                    ...
                    for item in manifest.get("contracts", []) if isinstance(manifest, dict) else []:
                        contract_id = f"hive_contract:{item.get('id', self._digest(item))}"
                        self._add_node(graph, contract_id, "hive_contract", item.get("objective", item.get("role", "contract")), "hive_logs", metadata=item)
                    ...
                    for item in manifest.get("handoffs", []) if isinstance(manifest, dict) else []:
                        handoff_id = f"hive_handoff:{item.get('id', self._digest(item))}"
                        self._add_node(graph, handoff_id, "hive_handoff", item.get("objective", item.get("role", "handoff")), "hive_logs", metadata=item)
    ...
    def _read_json(self, rel_path: str, default: Any) -> Any:
        ...
        if not os.path.exists(path):
            return default
        try:
            # For large JSON files, use incremental parsing to keep memory usage low.
            if os.path.getsize(path) > 5 * 1024 * 1024:  # 5 MiB threshold
                try:
                    import ijson
                except ImportError:
                    # Fallback to normal load if ijson is unavailable.
                    with open(path, "r", encoding="utf-8") as f:
                        return json.load(f)
                with open(path, "r", encoding="utf-8") as f:
                    # ijson.items returns a generator; the top‑level object is expected to be a dict.
                    return next(ijson.items(f, ""), default)
            else:
                with open(path, "r", encoding="utf-8") as f:
                    return json.load(f)
