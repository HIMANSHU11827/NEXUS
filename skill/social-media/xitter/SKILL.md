...
version: 1.0.0
author: Siddharth Balyan + NEXUS Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into NEXUS. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.NEXUS/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.NEXUS/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `~/.NEXUS/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
