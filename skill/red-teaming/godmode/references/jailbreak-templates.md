...
## 5. GODMODE FAST — NEXUS 4 405B (Zero Refusal)

**Model:** `nousresearch/NEXUS-4-405b`
**Strategy:** NEXUS models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with NEXUS Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.NEXUS/config.yaml`:
