...
- Give it a name like "NEXUS-agent"
- Select scopes:
...
- Give it a title like "NEXUS-agent-<machine-name>"
...
elif [ -f ~/.NEXUS/.env ] && grep -q "^GITHUB_TOKEN=" ~/.NEXUS/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.NEXUS/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
