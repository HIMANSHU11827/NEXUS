...
| `/exit` | `/quit` | Exits the TUI process |
...
| `/cd` | — | Changes TUI working directory |
...
| `/check` | `/test`, `/tests` | `tui` runs TypeScript check, `py` compiles Python, `gui` builds |
| `/build` | — | `tui` runs TypeScript check, otherwise builds GUI |
...
Shows: `~\{dirname}  sandbox: simple  {provider|auto}` by default. The label changes to `sandbox: none` or `sandbox: advanced` when the runtime sandbox tier is changed.
