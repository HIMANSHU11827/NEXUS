...
- **nexus/** — Boot loader (`boot()` function). Entry point for `python -m nexus`, `--setup`, `--server`, `--gui`, `--shell`, `--tui`. Default route launches the Ink TUI and starts the canonical `apps.api:app` API when needed.
...
- **commands/** — Central command system (`CommandRegistry` singleton in `nexus/commands.py`). One registry shared by CLI, TUI, GUI, API, gateways, Hive, workflows, and automation. Every command resolves to exactly one handler.
...
- Backward-compat shims `server/`, `tui/`, `gui/` remain as thin re-exports so existing imports keep working during transition.
...
- **providers/** — Backward-compat re-export shim so the 269+ pre-existing `from providers.X` imports and the dynamic factory keep working. Do not add new code here.
...
Registry-discovered tools, each a flat named directory: `code_search/`, `creating/`, `deep_research/`, `deleting/`, `git_ops/`, `hive/`, `knowledge/`, `memory/`, `modifying/`, `planning/`, `reading/`, `reasoning/`, `shortcuts/`, `system/`, `task/`, `terminal/`, `test_runner/`, `web_search/`. Planning is a **tool** (`extensions/tools/built_in/planning/`), not `src/nexus/planning/` (mandate §2.3). `tools/` is now a backward-compat shim.

## Extension Systems

- **extensions/mcp/core/** — MCP stdio client/server and tool adapter (security boundary rejects workspace-root escape, bounds result limits). `mcp/` is a backward-compat shim.
- **extensions/plugins/built_in/** — Plugin discovery/execution with trust model. `plugins/` is a backward-compat shim.
- **extensions/skills/built_in/** — Skills as `SKILL.md` packages with `SkillRegistry`. `skills/` is a backward-compat shim.
...
- **permissions/** — Permission modes (auto, plan, acceptEdits, dontAsk, bypass, approve).
...
- **reasoning/** — Hyper-reasoning engine for planner/critic/verifier.
...
4. Events validated through `CanonicalEvent.from_work_event()` in `nexus/events.py`.
5. Events persisted to `workspace/work_events/<session>.jsonl`.
