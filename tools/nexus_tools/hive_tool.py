...
class HiveSpawnTool(BaseTool):
    ...
    Allows an agent to delegate work by spawning a sub-agent.
    """
    ...
    def __init__(self, root_dir: str):
        ...
        self.description = (
            ...
            "Params: 'objective' (str), 'persona' (str - e.g. ENGINEER, AUDITOR), 'hive_id' (opt)."
        )
    ...
    def call(self, **kwargs) -> ToolResult:
        ...
        kernel = get_nexus_kernel()
        result = kernel.hive.spawn_agent(objective, persona=persona, hive_id=hive_id)
