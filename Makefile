...
# Python (3.12+, uv) is the primary toolchain. uv sync builds & installs the
# `nexus` package + console commands, so `nexus`/`python -m nexus` work after it.
...
.PHONY: help install setup test lint api web tui clean

help: ## List available targets
	@grep -E '^[a-zA-Z_-]+:.*##' $(MAKEFILE_LIST) | awk -F ':.*##' '{printf "%-12s %s\n", $$1, $$2}' | sort

install: ## Sync Python deps + install the nexus package (console commands)
	uv sync
