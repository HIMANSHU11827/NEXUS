"""NEXUS durable task queue package (imported as ``queues``).

NOTE: this package is named ``queues`` and, when the repo root is on
``sys.path`` (e.g. running from the project directory), forwards any name it
does NOT define (Queue, Empty, LifoQueue, ...) to the real stdlib ``queue``
module so other libraries (urllib3 -> queue.LifoQueue) keep working.
"""
